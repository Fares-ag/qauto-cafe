# qauto-cafe

Multi-branch café POS and back-office platform — NestJS API, Next.js web, Supabase Postgres. The web app polls the API, which reads Postgres directly on every request.

## Stack

- **API** — NestJS, Prisma, Socket.IO (`apps/api`)
- **Web** — Next.js 15 (`apps/web`)
- **Database** — PostgreSQL (Supabase session pooler)
- **Cache / jobs** — none on the server; the web app polls the API, which reads Postgres directly

## Local development

```bash
npm ci
docker compose up -d          # Postgres (optional if using Supabase)
cp .env.example .env          # configure DATABASE_URL, JWT secrets
npm run db:generate
npx prisma migrate deploy
npm run db:seed
npm run dev                     # API :3001 + Web :3000
```

Default login: `admin@qauto.com` / `admin123` (PIN `1234`).

## Production deploy

| Service | Host | Config |
|---------|------|--------|
| Web | [Vercel](https://vercel.com) (recommended) | Root dir `apps/web`, see `apps/web/vercel.json` |
| Web (alt) | Railway | Config file `apps/web/railway.toml` — **not** root `railway.toml` |
| API | [Railway](https://railway.app) | Root `railway.toml` or `apps/api/railway.toml` — must use **Dockerfile** builder |
| Postgres | Supabase | `deploy/railway.api.env.example` |

**Railway monorepo:** each service must set its own **Config file path** in Settings. There is no root `railway.toml` — using the wrong path builds the wrong Dockerfile.

See `deploy/vercel.env.example` and `deploy/railway.api.env.example` for environment variables.

## Tests

```bash
npm run test --workspace=@qauto/api
cd apps/web && npx playwright test
```

## License

Private — QAuto Café
