  ...
  async getStock(@CurrentUser() user: AuthenticatedUser, @Query('branchId') branchId: string) {
    ...
    const layers = await this.prisma.stockLayer.findMany({
      where: {
        ...
        ingredient: { organizationId: user.organizationId },
      },
      select: { ingredientId: true, quantityRemaining: true, unitCost: true },
    });

    const stats = new Map<string, { qty: Prisma.Decimal; value: Prisma.Decimal }>();
    for (const layer of layers) {
      const entry = stats.get(layer.ingredientId) ?? {
        qty: new Prisma.Decimal(0),
        value: new Prisma.Decimal(0),
      };
      entry.qty = entry.qty.add(layer.quantityRemaining);
      entry.value = entry.value.add(layer.quantityRemaining.mul(layer.unitCost));
      stats.set(layer.ingredientId, entry);
    }
